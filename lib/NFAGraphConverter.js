...
class NFAGraphConverter {
    ...
    convert(nfa) {
        ...
        return new DFARunner(this.generateTable());
    }

    getEpsilonReachable(nfa_node) {
        var result = new ArraySet();

        // we can always get to the node from where we're starting
        result.add(nfa_node);

        // keep expanding the result set, adding epsilon reachable nodes for
        // each new node added to the set. Note that the list may grow with
        // each iteration, so we can't cache the node list length nor use
        // forEach
    ...
    generateTable() {
        ...
        function emitMORLE(dfa_node) {
            ...
            for (var i = 0; i < 256; i++) {
                var ch = String.fromCharCode(i);
                var target_node = dfa_node.getTransition(ch);
                var value = (target_node === null) ? -1 : target_node.id;
        ...
        this.dfa_nodes.forEach(dfa_node => {
            var acceptState = dfa_node.acceptState;

            // Minus One Run-Length Encoding
            var entry = emitMORLE(dfa_node);

            table.push([entry, acceptState]);
        });
